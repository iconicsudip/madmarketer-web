...
export default function ServicesEcosystem({ initialServices = [] }: { initialServices?: any[] }) {
  const [activeFilter, setActiveFilter] = useState("All");
  const containerRef = useRef<HTMLDivElement>(null);
  const trackRef = useRef<HTMLDivElement>(null);

  const activeData = initialServices.length > 0 ? initialServices : fallbackServices;

  const filteredServices = activeFilter === "All" 
    ? activeData 
    : activeData.filter(s => s.category === activeFilter);
